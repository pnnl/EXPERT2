...
from src.model_io_textwithstruct import create_checkpoint_directories, load_or_initialize_atlas_model, load_or_initialize_atlas_model_forindex
...
def evaluate(model, index, opt, data_path, step=None):
    ...
    for i, batch in enumerate(data_iterator):
        ...
        # logger.info(f"## retrieved passage, {len(retrieved_passages[0])}")
        ...
        # )
        # logger.info(f"eval loss/logits comp done:{batch_struct_emb_training.shape, len(query)}")
        ## edit by sai
...
if __name__ == "__main__":
    ...
    for eval_file in opt.eval_data:
        ...
        model, _, _, _, _, opt, step, _, _, _, _ = load_or_initialize_atlas_model(opt, eval_only=True)
        ...
        for name,group in grouped:
            ...
            try:
                if opt.retrieve_only:
                    ...
                    # logger.info("loaded subset index")
