...
from src.model_io_textwithstruct import create_checkpoint_directories, load_or_initialize_atlas_model, save_atlas_model
...
def train(
    ...
    while step < opt.total_steps:
        ...
        logger.info(f"train data path:{opt.train_data}")
        ...
        for i, batch in enumerate(data_iterator):
            ...
            step += 1
            count_step += 1 # stopping train function
            ...
            # logger.info(f"loss comp done:{reader_loss,retriever_loss,retriever_graph_loss}")
            logger.info(f"loss comp done")
            ...
            if step % opt.accumulation_steps == 0 and not stats["skip_example"]:
                if opt.is_distributed and opt.shard_optim:
                    ...
                    proj_optimizer.clip_grad_norm(scale * opt.clip)
                ...
                ## projector weights update
                proj_optimizer.step(scale=scale) 
                proj_scheduler.step()
...
if __name__ == "__main__":
    ...
    model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt, step, \
        retr_graph_optimizer, retr_graph_scheduler,proj_optimizer, proj_scheduler = load_or_initialize_atlas_model(
            opt)
    ...
    if opt.is_distributed:
        if opt.shard_grads:
            ...
            model.projector = fairscale.nn.data_parallel.ShardedDataParallel(
                model.projector, proj_optimizer, auto_refresh_trainable=False
            )
            if opt.train_retriever:
    ...
    ## edit
    ## iterate over domain
    for repeat_domain in range(82):
        logger.info(f"repeated domain:{repeat_domain}")
        
        for count_domain in range(4):

            if count_domain==0:
                opt.subset_textindex_name="Physics,Chemistry,Bio-1,Bio-2,Mathematics,Art,Environmental-Science"
                opt.train_data = "traindata_path/PhyScience_v1.jsonl"
                ...
                opt.subset_textindex_name = "Art,Geography,History,Sociology,Philosophy,Political-Science,Psychology,Business,Economics,Geology,Bio-1,Engineering"
                opt.train_data = "traindata_path/HSS.jsonl"
                ...
                opt.subset_textindex_name = "Computer-Science,Environmental-Science,Engineering,Materials-Science,Mathematics,Art,Bio-1"
                opt.train_data = "traindata_path/Engg.jsonl"
                ...
                opt.subset_textindex_name = "Med-1,Med-2,Art,Environmental-Science,Bio-1"
                opt.train_data = "traindata_path/Med-1.jsonl"
