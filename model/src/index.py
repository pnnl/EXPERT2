...
class DistributedIndex(object):
    ...
    def init_embeddings(self, passages, dim: Optional[int] = EMBEDDINGS_DIM):
        ...
        self.embeddings = torch.zeros(
            dim, (len(passages)), dtype=torch.float16)
        if self.is_in_gpu:
    ...
    def save_index(self, path: str, total_saved_shards: int, overwrite_saved_passages: bool = False) -> None:
        ...
        #       total_saved_shards, ws, embeddings_per_shard)
        assert n_embeddings == len(self.doc_map), len(self.doc_map)
        ##### edited by sai ####
        # embeddings_per_shard = 1
    ...
    def load_index(self, path: str, total_saved_shards: int, load_subset_textindex: bool, subset_textindex_name: str, passage_path: str):
        """
        Loads sharded embeddings and passages files (no index is loaded).
        """
        rank = dist_utils.get_rank() # ws =16 with rank from 0 to 16 for each GPU device
        ws = dist_utils.get_world_size() # total saved shards =128
        assert total_saved_shards % ws == 0, f"N workers must be a multiple of shards to save"
        shards_per_worker = total_saved_shards // ws
        passages = []
        embeddings = []
        # logger.info(f"shards:{rank, ws, total_saved_shards}")
        # print("## load index range", rank * shards_per_worker, (rank + 1) * shards_per_worker)

        ##### edit by sai ####
        # count_shards = 0
        ## if load subset of index 

        if load_subset_textindex:
            
            for ele_indexname in subset_textindex_name.split(","):

                logger.info(f"subset index per worker:{ele_indexname,shards_per_worker}")
                domain_path = os.path.join(path, ele_indexname)
                passage_domain_path = os.path.join(passage_path, ele_indexname)

                for shard_id in range(rank * shards_per_worker, (rank + 1) * shards_per_worker):
                    # count_shards = count_shards + 1
                    # passage_shard_path = self._get_saved_passages_path(domain_path, shard_id)
                    passage_shard_path = self._get_saved_passages_path(passage_domain_path, shard_id)
                    # logger.info(f"psg path:{passage_shard_path}")
                    try:
                        ...
                        embeddings_shard_path = self._get_saved_embedding_path(
                            domain_path, shard_id)

                        embeddings.append(torch.load(
                            embeddings_shard_path, map_location="cpu").cuda())
                                                
                        # logger.info(f"embd path:{embeddings_shard_path}")
                        # logger.info(f"embd value:{embeddings[0]}")
                    except:
                        continue
        ...
        #     self.embeddings = torch.concat(embeddings, dim=1)
        # except:
        #     new_emb = torch.rand(size=(768,1000))
        #     self.embeddings = new_emb.cuda()
        #     logger.info("random index")

    def gen_representing_index(self, path: str, total_saved_shards: int, load_subset_textindex: bool, ele_indexname: str):
        """
        Loads sharded embeddings and passages files (no index is loaded).
        """
        rank = dist_utils.get_rank() # ws =16 with rank from 0 to 16 for each GPU device
        ...
        if load_subset_textindex:
            ...
            tmp_emb = torch.empty((768,1))
            logger.info(f"subset index:{ele_indexname}")
            domain_path = os.path.join(path, ele_indexname)
            ...
            for shard_id in range(rank * shards_per_worker, (rank + 1) * shards_per_worker):
                ...
                try:
                    ...
                    tmp_emb = torch.cat((tmp_emb,torch.load(
                        embeddings_shard_path, map_location="cpu")), 1)
            ...
            # save domain wise index
            logger.info(f"tmp embd:{tmp_emb.shape}")
            mean_emb = torch.mean(tmp_emb, 1, True)
            max_emb, max_idxs = torch.max(tmp_emb, 1, True)
            # logger.info(f"mean embd:{mean_emb}")
            # logger.info(f"mean embd sh:{mean_emb.shape}")
            with open(domain_path+'/mean_emb.pt','wb') as f:
                torch.save(mean_emb, f)
            with open(domain_path+'/max_emb.pt','wb') as f:
                torch.save(max_emb, f)
    ...
    ## compute similarity search and return top-k domain index
    def compute_scores_and_domainindices(self, allqueries: torch.tensor, domain_wise_index: torch.tensor, topk: int):
        """
        Computes the distance matrix for the query embeddings and embeddings chunk and returns the k-nearest neighbours and corresponding scores.
        """
        # logger.info(f"data type: {allqueries.dtype,domain_wise_index.dtype}")
        # logger.info(f"data half type: {allqueries.half().dtype,domain_wise_index.half().dtype}")
        # scores = torch.matmul(allqueries, domain_wise_index)
        # logger.info(f"scores: {allqueries,domain_wise_index,scores.shape,scores}")
        # scores, indices = torch.topk(scores, topk, dim=1)
        # return scores, indices
        ...
        scores_matrix = torch.empty((allqueries.shape[0],domain_wise_index.shape[1]))
        # logger.info(f"scores shape: {scores_matrix.shape}")
        for ele1 in range(allqueries.shape[0]):
            for ele2 in range(domain_wise_index.shape[1]):
                scores_matrix[ele1,ele2] = compute_cosine_similarity(allqueries[ele1,:],domain_wise_index[:,ele2] )

        scores, indices = torch.topk(scores_matrix, topk, dim=1)

        return scores, indices
    
    #### search domain from the query embeddings
    @torch.no_grad()
    def search_knn_domain(self, queries, domain_wise_index_vec, domain_map_names, topk):
        """
        ...
        domain_wise_index_vec = domain_wise_index_vec.to(allqueries.device)
        scores, indices = self.compute_scores_and_domainindices(allqueries, domain_wise_index_vec, topk)
        indices = indices.tolist()
        indices_list = [ele for temp in indices for ele in temp]
        # logger.info(f"indices list {indices_list}")
        frequency = dict(collections.Counter(indices_list))
        # final_index_list = sorted(frequency, key=frequency.get, reverse=True)
        final_index_list = sorted(frequency, key=frequency.get, reverse=True)[0:topk]
        logger.info(f"sel domain indices {final_index_list}")
        domain_names_list = [domain_map_names[x] for x in final_index_list]
    ...
    def search_knn(self, queries, topk):
        ...
        docs = [[self.doc_map[x] for x in sample_indices]
                for sample_indices in indices]

        # print("## entry docs rank", len(docs[0]), len(
        #     scores[0]), len(indices[0]), indices[0:2])
        if torch.distributed.is_initialized():
            docs = [docs[allsizes[k]: allsizes[k + 1]]
                    for k in range(len(allsizes) - 1)]
            docs = [serialize_listdocs(x) for x in docs]
            scores = [scores[allsizes[k]: allsizes[k + 1]]
                      for k in range(len(allsizes) - 1)]
            gather_docs = [dist_utils.varsize_gather(
                docs[k], dst=k, dim=0) for k in range(dist_utils.get_world_size())]
            gather_scores = [
        ...
        scores = [[scores[k][j] for j in idx]
                  for k, idx in enumerate(subindices)]
        docs = [[docs[k][j] for j in idx] for k, idx in enumerate(subindices)]
        # print("## after docs rank", len(docs), docs[0], len(
        #     scores), len(subindices), subindices[0:2])
        return docs, scores
...
class DistributedFAISSIndex(DistributedIndex):
    ...
    def train_index(self) -> None:
        ...
        if isinstance(self.faiss_gpu_index, FAISSGPUIndex.__args__):
            self.faiss_gpu_index.train(
                self._cast_to_torch32(self.embeddings.T))
        else:
