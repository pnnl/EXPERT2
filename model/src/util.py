...
def set_optim(opt, model):
    ...
    if opt.train_retriever & opt.retrieve_with_rerank_bygraph:
        retr_graph_optimizer = optim_class(params=model.retriever_graph.parameters(), lr=opt.lr_retriever, **optim_args)
    
    if opt.projector!=None:
        proj_optimizer = optim_class(params=model.projector.parameters(), lr=opt.lr, **optim_args)

    retr_scheduler = None
    retr_graph_scheduler = None
    proj_scheduler = None
    ...
    if opt.train_retriever & opt.retrieve_with_rerank_bygraph:
        retr_graph_scheduler = scheduler_class(retr_graph_optimizer, **scheduler_args)
        
    if opt.projector!=None:
        proj_scheduler = scheduler_class(proj_optimizer, **scheduler_args)
        
    return optimizer, scheduler, retr_optimizer, retr_scheduler, retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler
