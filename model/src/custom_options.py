...
class Options:
    ...
    def initialize_parser(self):
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="none",
            help="Path to a pretrained model for index search",
        )
        self.parser.add_argument(
            "--domain_rep_vector_path",
            type=str,
            default="none",
            help="Path to a representing vector from each domain)",
        )
        self.parser.add_argument(
            "--per_gpu_batch_size",
            default=2,
        ...
        self.parser.add_argument(
            ...
            default=1,
            type=int,
            help="Batch size per GPU/CPU for search domain index.",
        )
        self.parser.add_argument(
            "--no_sel_indices",
            default=3,
            type=int,
            help="no of selected indices for input to retriever.",
        )
        ...
        self.parser.add_argument(
            "--write_results", action="store_true", help="save evaluation results to file")
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="",
            help="Path to adocument graph",
        )
        self.parser.add_argument(
            "--subset_textindex_name",
            type=str,
            default="",
            help="Path to subset index names separated by comma without space",
        )
        self.parser.add_argument(
            "--subset_passage_name",
            type=str,
            default="",
            help="Path to subset passage names",
        )

    def add_optim_options(self):
        self.parser.add_argument(
            "--warmup_steps", type=int, default=5, help="number of learning rate warmup steps")
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--accumulation_steps", type=int, default=1, help="gradient accumulation")
        self.parser.add_argument(
            "--dropout", type=float, default=0.1, help="dropout rate")
        self.parser.add_argument(
            "--lr", type=float, default=4e-5, help="learning rate")
        self.parser.add_argument(
            "--lr_retriever", type=float, default=1e-5, help="learning rate for retriever")
        self.parser.add_argument("--clip", type=float,
        ...
        self.parser.add_argument(
            "--epsilon", type=float, default=1e-6, help="adamw epsilon value")
        self.parser.add_argument(
            "--alpha", type=float, default=1.0, help="adamw alpha value")
        self.parser.add_argument(
            "--beta2", type=float, default=0.999, help="adamw beta2 value")
        self.parser.add_argument(
            "--refresh_index",
            type=str,
            default="-1",
        ...
        self.parser.add_argument(
            "--shuffle", action="store_true", help="shuffle data for training")
    ...
    def add_modeling_options(self):
        ...
        self.parser.add_argument(
            "--passages_subdir",
            nargs="+",
            help="list of main dir of paths to jsonl files containing passages to index and retrieve from. Unused if loading a saved index using --load_index_path",
        )
        self.parser.add_argument(
            "--max_passages",
            type=int,
        ...
        #### edited by sai ####
        self.parser.add_argument(
            "--retriever_n_context_beforererank",
            type=int,
            default=50,
            help="number of top k passages to use for reranking",
        )
        self.parser.add_argument(
            "--gold_score_mode",
            type=str,
            choices=["evalnormsum", "loop",
                     "ppmean", "emdr", "pdist", "adist"],
            default="pdist",
        ...
        self.parser.add_argument(
            "--compute_crossattention_stats", action="store_true")
        self.parser.add_argument(
        ...
        self.parser.add_argument("--freeze_retriever_steps", type=int,
                                 default=-1, help="freezes retriever for n steps")
        self.parser.add_argument(
        ...
        #### edited by sai ###
        self.parser.add_argument(
            "--retrieve_with_rerank_bygraph",
            action="store_true",
            help="pass this to enable reranking with graph neural network on passage graph",
        )
        self.parser.add_argument(
            "--projector",
            action="store_true",
            help="pass this to enable projection from low dim struct embedding to higher dim to match text emb",
        )
        self.parser.add_argument(
            "--load_subset_textindex",
            action="store_true",
            help="pass this to enable loading a subset of text passage index",
        )
        self.parser.add_argument(
            "--gen_textdomain_representation_vec",
            action="store_true",
            help="pass this to enable generating a representation vector for text domain index",
        )
        self.parser.add_argument(
            "--gen_passages_embeddings",
            action="store_true",
            help="pass this to enable creating embeddings for each passages domain",
        )
        ...
        # Generation options
        self.parser.add_argument(
            "--generation_max_length", type=int, default=128)
        self.parser.add_argument(
            "--generation_min_length", type=int, default=None)
        self.parser.add_argument(
            "--generation_length_penalty", type=float, default=1.0)
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="base",
            choices=["base", "mlm", "lm", "multiple_choice",
                     "kilt", "section", "fever", "qa"],
        ...
        type=int,
        default=768,
        help="passage embeddings dim from retriever",
        )
        self.parser.add_argument(
        "--node_hidemb_size",
        type=int,
        default=128,
        help="hidden dim of GNN network",
        )
    ...
    def add_index_options(self):
        ...
        self.parser.add_argument("--faiss_code_size", type=int,
                                 default=None, help="Parameter for PQ/SQ quantization")
