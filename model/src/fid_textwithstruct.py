...
from src.custom_modeling_t5 import T5ForConditionalGeneration, T5Stack
...
class FiDStack(T5Stack):
    ...
    def forward(
        ...
        if not self.is_decoder:
            ...
            # logger.info(f"batch struct emb shape 58:{batch_struct_emb.shape}")
        # logger.info(f"encoder attn mask fid 55:{encoder_attention_mask}")
        
        ## calls T5stack fornward in modeling_t5.py
        ...
        if not self.is_decoder:
            # logger.info(f"check dict:{return_dict}")
            if not return_dict:
                ...
                ##TODO 
        # logger.info(f"## output Fidstack:{output.shape}")
        # print("## output Fidstack",len(output))
        return output
...
class FiD(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        # logger.info(f"## self encoder:{self.encoder}")
        ...
        # logger.info(f"## self decoder:{self.decoder}")
        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False)
        # logger.info(f"## self head:{self.lm_head}")
    ...
    def get_crossattention_scores(self, n_passages, mask, labels, ids, mode="all", mask_query=None):
        ...
        # logger.info(f"## output cross attn:{len(output)}")
        return output
    ...
    def aggregate_value(self, scores, mask, labels, n_passages, ids, mask_query=None, output={}, prefix=""):
        ...
        # logger.info(f"## output FID:{len(output)}")
        return output
...
def cross_attention_forward(
    ...
    # logger.info(f"## output FID:{len(outputs)}")
    return outputs
...
def traceit(frame, event, arg):
    if event == "line":
        ...
        try:
            ...
            if ("src.modeling_t5" in name) or ("src.fid" in name):
