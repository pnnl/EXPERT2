...
import src.fid_textwithstruct
from src import dist_utils
from src.atlas_textwithstruct import Atlas
...
def load_reader(opt):
    ...
    if not opt.retrieve_only:
        reader = src.fid_textwithstruct.FiD.from_pretrained(opt.reader_model_type)
...
def load_atlas_model(dir_path, opt, reset_params=False, eval_only=False,projector_flag=False):
    ...
    model = Atlas(opt, reader, retriever,
                  reader_tokenizer, retriever_tokenizer, retriever_graph, projector)

    # TODO
    model = _load_atlas_model_state(opt, opt_checkpoint, model, model_dict)

    if eval_only:
        return model, None, None, None, None, opt_checkpoint, step, None, None, None, None
    ...
    return model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt_checkpoint, step, retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler
...
def init_atlas_model(opt, eval_only):
    ...
    if eval_only:
        return model, None, None, None, None, opt, 0, None, None,None,None

    optimizer, scheduler, retr_optimizer, retr_scheduler, retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler = set_optim(
        opt, model)
    return model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt, 0, retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler
...
def load_or_initialize_atlas_model_forindex(opt, eval_only=False):
    ...
    model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt_checkpoint, loaded_step, \
        retr_graph_optimizer, retr_graph_scheduler , proj_optimizer, proj_scheduler = load_atlas_model(
            load_path, opt, reset_params=reset_params, eval_only=eval_only, projector_flag=False,
        )
...
def load_or_initialize_atlas_model(opt, eval_only=False):
    ...
    model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt_checkpoint, loaded_step, \
        retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler = load_atlas_model(
            load_path, opt, reset_params=reset_params, eval_only=eval_only, projector_flag=projector_flag,
        )
    logger.info(f"Model loaded from {load_path}")
    step = 0 if opt.model_path != "none" else loaded_step

    return model, optimizer, scheduler, retr_optimizer, retr_scheduler, opt, step, retr_graph_optimizer, retr_graph_scheduler, proj_optimizer, proj_scheduler

def save_atlas_model(model, optimizer, scheduler, retr_optimizer, retr_scheduler, step, opt, dir_path, name,
                     retr_graph_optimizer, retr_graph_scheduler, proj_optimizer,proj_scheduler):
    ...
    checkpoint = {
        ...
        "proj_optimizer": proj_optim_state,
        "proj_scheduler": proj_scheduler.state_dict(), 
    }
