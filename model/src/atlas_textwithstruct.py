...
class Atlas(nn.Module):
    def __init__(self, opt, reader, retriever, reader_tokenizer, retriever_tokenizer, retriever_graph, projector):
    ...
    def perplexity_score(self, reader_ids, reader_mask, decoder_input_ids, labels, cfg, bsz,batch_struct_emb=None):
        with torch.no_grad():
            self.reader.eval()
    ...
    def forward(
        ...
        # logger.info(f"len reader ids 4 forward pass:{reader_ids_training.shape,reader_mask_training.shape,decoder_input_ids.shape,batch_struct_emb_training.shape}")
    ...
    @ torch.no_grad()
    def compute_reader_loss_and_logits(self, tokens, decoder_input_ids, labels, batch_struct_emb=None):
        cfg = self.reader.encoder.config
    ...
    @ torch.no_grad()
    def generate(self, tokens, query, choices=None, batch_struct_emb=None):
        cfg = self.reader.encoder.config
        cfg.bsz = tokens["input_ids"].size(0)
        cfg.n_context = min(self.opt.n_context, tokens["input_ids"].size(1))
    ...
    ## edit by sai
    @torch.no_grad()
    def projector_output(self,batch_struct_emb_tensor_training):
        return self.projector(batch_struct_emb_tensor_training)
